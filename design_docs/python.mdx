...
```python Async Client (Recommended)
from olostep import AsyncOlostep

client = AsyncOlostep(api_key="YOUR_API_KEY")
```
...
batch = await client.batches.create(urls=["https://site1.com", "https://site2.com"])
...
crawl = await client.crawls.create(start_url="https://example.com", max_pages=100)
...
```python
from olostep import AsyncOlostep, Format, Country, WaitAction, FillInputAction
...
```python
from olostep import AsyncOlostep, Country
...
batch = await client.batches.create(
    [
        {"url": "https://www.google.com/search?q=olostep", "custom_id": "search_1"},
        {"url": "https://www.google.com/search?q=olostep+api", "custom_id": "search_2"}
    ],
    country=Country.US,
...
async for item in client.batches.items(batch_id='a_batch_id', batch_size=10):
...
# Alternative: Direct API access (stateless)
async for page in client.crawls.pages(crawl_id='a_crawl_id'):
    content = await page.retrieve(["html"])
    print(f"{page.url}: {len(content.html_content)} bytes")
```
...
await client.scrapes.create(url_to_scrape="https://example.com")     # explicit method

await client.batches.create(urls=["url1", "url2"])                   # explicit method

await client.crawls.create(start_url="https://example.com")          # explicit method

await client.maps.create(url="https://example.com")                  # explicit method
