"""
Synchronous facade over the async Olostep client (new architecture).
...
from .async_client import AsyncOlostepClient


class _SyncProxy:
    def __init__(self, outer: "OlostepClient", resolver: Callable[[AsyncOlostepClient], Any], dir_list: list[str] | None = None) -> None:
        self._outer = outer
...
class OlostepClient:
    def __init__(self, api_key: str | None = None, base_url: str | None = None) -> None:
        ...
        self.map = _SyncProxy(self, lambda c: c.map, dir_list=["create"])
        self.retrieve = _SyncProxy(self, lambda c: c.retrieve, dir_list=["get"])

    def __dir__(self) -> list[str]:
        return ["scrape", "batch", "crawl", "map", "retrieve"]
    ...
    def _call(self, func: Callable[[AsyncOlostepClient], Any]) -> Any:
        async def _inner():
            async with AsyncOlostepClient(api_key=self._api_key, base_url=self._base_url) as c:
                return await func(c)
